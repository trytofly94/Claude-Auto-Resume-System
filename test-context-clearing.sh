...
# Test Script for Context Clearing Implementation (Issue #93)
# Validates the core functionality without requiring full system integration
...
# Test 5: Context clearing decision logic (requires hybrid-monitor functions)
test_decision_logic() {
    # Load hybrid monitor for decision functions
    if ! source "$SCRIPT_DIR/src/hybrid-monitor.sh" 2>/dev/null; then
        echo "Could not load hybrid-monitor.sh for decision logic test"
        return 1
    fi
